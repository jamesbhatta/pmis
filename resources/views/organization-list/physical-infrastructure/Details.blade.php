        ...
        <div class="container mt-3">
            <div class="row">
                <div class="row col-6">
                    ...
                    <div class="row listOfDetails">
                        <ul>
                            <li>लागत अनुमान बनेको छ?:@if ($project->physicalProgress == '')
                                    @else{{ $project->physicalProgress->estimate_completed ? 'छ' : 'छैन' }}
                                @endif
                            </li>
                            <li>सम्झौता भएको छ?: @if ($project->physicalProgress == '')
                                    @else{{ $project->physicalProgress->agreement_date ? 'छ' : 'छैन' }}
                                @endif
                            </li>
                            <li>सम्झौता भएको मिति:@if ($project->physicalProgress == '')
                                    @else{{ $project->physicalProgress->agreement_date }}
                                @endif
                            </li>
                            <li>परियोजना सुरु मिति:@if ($project->physicalProgress == '')
                                    @else{{ $project->physicalProgress->project_start_date }}
                                @endif
                            </li>
                            <li>परियोजना सम्पन्न मिति:@if ($project->physicalProgress == '')
                                    @else{{ $project->physicalProgress->project_completion_date }}
                                @endif
                            </li>
                            <li>ठेक्का मिति:@if ($project->physicalProgress == '')
                                    @else{{ $project->physicalProgress->tender_date }}
                                @endif
                            </li>
                            <li>काम हुदै छ?:@if ($project->physicalProgress == '')
                                    @else{{ $project->physicalProgress->wip ? 'छ' : 'छैन' }}
                                @endif
                            </li>
                            <li>अनुगमन हुदै छ?@if ($project->physicalProgress == '')
                                    @else{{ $project->physicalProgress->followed_up ? 'छ' : 'छैन' }}
                                @endif
                            </li>
                        </ul>
                    </div>
                ...
                <div class="row col-6" style=" width:100%;float:right">
                    ...
                    <div class="container">
                        <canvas id="myBarChart"
                            style="height: 300px; max-width: 640px; margin: 0px auto; opacity:2;"></canvas>
                    </div>
                    <div class="row" style="width: 100%; float:left;">
        ...
        const myChart = new Chart(barChart, {
            ...
            data: {
                ...
                datasets: [{
                    ...
                    data: [{{ $project->last_year_physical_progress ? '0' : $project->last_year_physical_progress }},
                        progress, 0, 100
                    ],
                }]
            },
